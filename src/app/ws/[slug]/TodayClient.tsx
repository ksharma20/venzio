...
function RealtimeWidget({ data, loading, activeCount, locationCounts }: { data: RealtimeResponse | null; loading: boolean; activeCount?: number; locationCounts?: { label: string; count: number }[] }) {
...
// ─── Office Presence Graph ────────────────────────────────────────────────────

function OfficePresenceGraph({
  buckets,
  loading,
}: {
  ...
  // Use all 24 hourly buckets — filtering to UTC 9-18 misses data in non-UTC timezones
  const hourBuckets = [...buckets].sort((a, b) => parseInt(a.key) - parseInt(b.key))

  const rawMax = Math.max(...hourBuckets.map((b) => b.unique_users), 0)
  // Enforce a minimum scale of 4 so ticks are always distinct integers (0,1,2,3,4)
  ...
  const pts = hourBuckets.map((b, i) => {
    ...
    // Show label as "9AM", "12PM" etc — only every 2nd to avoid crowding
    const ampm = h < 12 ? 'AM' : 'PM'
    const h12 = h === 0 ? 12 : h > 12 ? h - 12 : h
    const label = `${h12}${ampm}`
    return { x, y, label, count: b.unique_users, showLabel: h % 2 === 0 }
  })

  // Smooth cubic bezier through all points using cardinal spline control points
  ...
  // Integer Y-axis ticks: 0, step, 2×step, … up to yMax
      ...
      {loading ? (
        ...
        >
          ...
          {hovered && (
            <g>
              <line
                x1={hovered.x.toFixed(1)} y1={padT.toFixed(1)}
                x2={hovered.x.toFixed(1)} y2={(padT + chartH).toFixed(1)}
              ...
              <rect
                x={(hovered.x - 28).toFixed(1)} y={(hovered.y - 34).toFixed(1)}
                width="56" height="22" rx="5"
                fill="var(--navy)" opacity="0.9"
              />
              <text
                x={hovered.x.toFixed(1)} y={(hovered.y - 18).toFixed(1)}
          ...
          {pts.map((p, i) => (
            <g key={i}>
              {/* X axis label — every 2nd hour only */}
              {p.showLabel && (
                <text
                  x={p.x.toFixed(1)} y={(padT + chartH + 18).toFixed(1)}
...
export default function TodayClient({ slug, workspaceName, planLimitBanner }: Props) {
  const [data, setData] = useState<DashboardResponse | null>(null)
  const [modal, setModal] = useState<{ title: string; members: DashboardMember[] } | null>(null)

  // Today hourly graph state
  ...
  const fetchDash = useCallback(async () => {
    try {
      const res = await fetch(`/api/ws/${slug}/dashboard?status=all&signal=all&sortBy=name&sortDir=asc&page=1&limit=200`)
  ...
  const officeCount = counts.office
  const remoteCount = counts.remote

  return (
    <div style={{ padding: '24px 24px', minHeight: '100%' }}>
      {/* Top header bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          onClick={async () => {
            ...
            a.href = url
            a.download = `report-${slug}-${y}-${m}.csv`
      ...
      {/* ── Office Presence Graph + Realtime Widget ── */}
      <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', alignItems: 'stretch', flexWrap: 'wrap' }}>
        <div style={{ flex: 2, minWidth: '300px' }}>
          <OfficePresenceGraph
            buckets={todayHourlyData?.buckets ?? []}
            loading={todayHourlyLoading}
          />
        </div>
      ...
      <MemberStatsTable
        ...
        interval={statsInterval}
        onIntervalChange={(iv) => setStatsInterval(iv)}
      />
